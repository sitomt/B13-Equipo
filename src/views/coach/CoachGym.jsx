...
import { Tag, CountBadge, ProgressRing, Skeleton, EmptyState } from '../../components/ui'
    ...
    <div className="space-y-6 pb-24">
      ...
      >
        {incid.loading ? (
          ...
          <div className="flex items-center gap-2 p-4 text-sage"><Check size={18} /><span className="text-sm font-semibold">Ninguna incidencia abierta</span></div>
        ) : (
          openIncid.map((i) => (
            <div key={i.id}>
              <div className="p-3">
      ...
      >
        {openMaint.length === 0 ? (
          <div className="flex items-center gap-2 p-4 text-sage"><Check size={18} /><span className="text-sm font-semibold">Nada roto pendiente</span></div>
        ) : (
          openMaint.map((i) => (
        ...
        <p className="px-4 py-2.5 text-xs text-ink/40">Antes de reportar algo roto, revisa si ya está aquí para no duplicar.</p>
      </SectionCard>
      ...
      >
        {ctpl.loading ? (
          ...
          <div className="p-3"><EmptyState icon={Spray} title="Sin ruta hoy" /></div>
        ) : (
          <>

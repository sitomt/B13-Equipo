...
// Banner compacto de atención: solo existe si hay avisos activos.
// Tocarlo lleva a la pantalla de avisos (no satura la pantalla del día).
export default function AlertsBanner({ anns = [], onOpen }) {
  if (!anns.length) return null
  const urgent = anns.filter((a) => a.priority === 'high').length
  const label =
    (urgent > 0 ? `${urgent} aviso${urgent > 1 ? 's' : ''} urgente${urgent > 1 ? 's' : ''}` : '') +
    (urgent > 0 && anns.length > urgent ? ' · ' : '') +
    (anns.length > urgent ? `${anns.length - urgent} aviso${anns.length - urgent > 1 ? 's' : ''}` : '')
  return (
    <button
    ...
    >
      ...
      <span className={`min-w-0 flex-1 truncate text-sm font-bold ${urgent > 0 ? 'text-white' : 'text-ink'}`}>{label}</span>
